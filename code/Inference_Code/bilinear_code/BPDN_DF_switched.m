function [varargout] = BPDN_DF_switched(varargin)

% [coef_dcs, recon_dcs, rMSE_dcs, PSNR_dcs] = ...
%        BPDN_DF_switched(MEAS_SIG, MEAS_SEL, DYN_FUN, DWTfunc, param_vals, ...
%        TRUE_VID)
%
%   The inputs are:
% 
...
for kk = 2:num_frames
    ...
    if lambda_historyb == 0
        if lambda_val <= lambda_b
            ...
            Af = @(x) [lambda_history*x(1:N2) - gamma_hat*f_dyn*x(N2+1:N2+N_b); ...
                   Phi(DWT_invert(x(1:N2)))];
            Ab = @(x) [DWT_apply(Phit(x(N2+1:end))) + lambda_history*x(1:N2);...
                   -gamma_hat*(f_dyn')*x(1:N2)];
            A = linop_handles([M+N2, N2+N_b], Af, Ab, opt_set);
            % Optimize the BPDN objective function with TFOCS
            res = solver_L1RLS( A, [zeros(N2,1); MEAS_SIG(:, :, kk)], lambda_val, zeros(N2+N_b, 1), opts );
            
%             [~,max_dyn] = max(abs(res(N2+1:N2+N_b)));                      % Find "best" dynamics function (explains the most)
%             f_dyn = DYN_FUN{max_dyn}*coef_dcs(:,:,kk-1);
%             Af  = @(x) [lambda_history*x(1:N2) - gamma_hat*f_dyn*x(N2+1); ...
%                            Phi(DWT_invert(x(1:N2)))];
%             Ab  = @(x) [DWT_apply(Phit(x(N2+1:end))) + lambda_history*x(1:N2);...
%                    -gamma_hat*(f_dyn')*x(1:N2)];
%             A   = linop_handles([M+N2, N2+1], Af, Ab, opt_set);
%             res = solver_L1RLS( A, [zeros(N2,1); MEAS_SIG(:, :, kk)], ...
%                         [lambda_val*ones(N2,1);0], zeros(N2+1, 1), opts ); % Optimize the BPDN objective function with TFOCS
        elseif lambda_val > lambda_b
            lam_rat   = (lambda_b/lambda_val);
            gamma_hat = lambda_history*(lambda_b/lambda_val);
            Af = @(x) [gamma_hat*x(1:N2) - lambda_history*f_dyn*x(N2+1:N2+N_b); ...
                   lam_rat*Phi(DWT_invert(x(1:N2)))];
            Ab = @(x) [lam_rat*DWT_apply(Phit(x(N2+1:end))) + ...
                   gamma_hat*x(1:N2); -lambda_history*(f_dyn')*x(1:N2)];
            A  = linop_handles([M+N2, N2+N_b], Af, Ab, opt_set);
            
            res = solver_L1RLS( A, [zeros(N2,1); MEAS_SIG(:, :, kk)], ...
                                       lambda_b, zeros(N2+N_b, 1), opts ); % Optimize the BPDN objective function with TFOCS
            
%             [~,max_dyn] = max(abs(res(N2+1:N2+N_b)));                      % Find "best" dynamics function (explains the most)
%             f_dyn = DYN_FUN{max_dyn}*coef_dcs(:,:,kk-1);
%             Af = @(x) [gamma_hat*x(1:N2) - lambda_history*f_dyn*x(N2+1:N2+1); ...
%                    lam_rat*Phi(DWT_invert(x(1:N2)))];
%             Ab = @(x) [lam_rat*DWT_apply(Phit(x(N2+1:end))) + gamma_hat*x(1:N2);...
%                    -lambda_history*(f_dyn')*x(1:N2)];
%             A = linop_handles([M+N2, N2+1], Af, Ab, opt_set);
%             
%             res = solver_L1RLS( A, [zeros(N2,1); MEAS_SIG(:, :, kk)],  ...
%                           [lambda_b*ones(N2,1);0], zeros(N2+1, 1), opts ); % Optimize the BPDN objective function with TFOCS
        end
        [~,max_dyn] = max(abs(res(N2+1:N2+N_b)));                      % Find "best" dynamics function (explains the most)
        f_dyn = DYN_FUN{max_dyn}*coef_dcs(:,:,kk-1);
        Af  = @(x) [lambda_history*x(1:N2) - lambda_history*f_dyn*x(N2+1); ...
                       Phi(DWT_invert(x(1:N2)))];
        Ab  = @(x) [DWT_apply(Phit(x(N2+1:end))) + lambda_history*x(1:N2);...
               - lambda_history*(f_dyn')*x(1:N2)];
        A   = linop_handles([M+N2, N2+1], Af, Ab, opt_set);
        ...
        if lambda_val <= lambda_b
            ...
            Af = @(x) [lb_h*x(N2+1:N2+N_b); lambda_history*x(1:N2) - ...
                  gamma_hat*f_dyn*x(N2+1:N2+N_b);Phi(DWT_invert(x(1:N2)))];
            Ab = @(x) [DWT_apply(Phit(x((N_b+N2+1):end))) + lambda_history*x((N_b+1):(N_b+N2)); ...
                   lb_h*x(1:N_b) - gamma_hat*(f_dyn')*x((N_b+1):(N_b+N2))];
            A = linop_handles([M+N2+N_b, N2+N_b], Af, Ab, opt_set);
            
            res = solver_L1RLS( A, [lambda_historyb*bcoef_dcs(:,:,kk-1);...
                       zeros(N2,1); MEAS_SIG(:, :, kk)], lambda_val, ...
                                                 zeros(N2+N_b, 1), opts ); % Optimize the BPDN objective function with TFOCS
            
            [~,max_dyn] = max(abs(res(N2+1:N2+N_b)));                      % Find "best" dynamics function (explains the most)
            f_dyn = DYN_FUN{max_dyn}*coef_dcs(:,:,kk-1);
            Af = @(x) [lb_h*x(N2+1:N2+1); ...
                   lambda_history*x(1:N2) - gamma_hat*f_dyn*x(N2+1:N2+1); ...
                   Phi(DWT_invert(x(1:N2)))];
            Ab = @(x) [DWT_apply(Phit(x((1+N2+1):end))) + lambda_history*x((1+1):(1+N2)); ...
                   lb_h*x(1:1) - gamma_hat*(f_dyn')*x((1+1):(1+N2))];
            A = linop_handles([M+N2+1, N2+1], Af, Ab, opt_set);
            % Optimize the BPDN objective function with TFOCS
            res = solver_L1RLS( A, [zeros(N2,1); MEAS_SIG(:, :, kk)],  [lambda_val*ones(N2,1);0], zeros(N2+1, 1), opts );
        elseif lambda_val > lambda_b
            lam_rat = (lambda_b/lambda_val);
            gamma_hat = lambda_history*(lambda_b/lambda_val);
            Af = @(x) [lambda_historyb*x(N2+1:N2+N_b); ...
                   gamma_hat*x(1:N2) - lambda_history*f_dyn*x(N2+1:N2+N_b); ...
                   lam_rat*Phi(DWT_invert(x(1:N2)))];
            Ab = @(x) [lam_rat*DWT_apply(Phit(x((N_b+N2+1):end))) + gamma_hat*x((N_b+1):(N_b+N2)); ...
                   lambda_historyb*x(1:N_b) - lambda_history*(f_dyn')*x((N_b+1):(N_b+N2))];
            A = linop_handles([M+N2+N_b, N2+N_b], Af, Ab, opt_set);
            % Optimize the BPDN objective function with TFOCS
            res = solver_L1RLS(A, [lambda_historyb*bcoef_dcs(:,:,kk-1); zeros(N2,1); MEAS_SIG(:, :, kk)], lambda_b, zeros(N2+N_b, 1), opts );
            
            [~,max_dyn] = max(abs(res(N2+1:N2+N_b)));                      % Find "best" dynamics function (explains the most)
            f_dyn = DYN_FUN{max_dyn}*coef_dcs(:,:,kk-1);
            Af = @(x) [lambda_historyb*x(N2+1:N2+1); ...
                   gamma_hat*x(1:N2) - lambda_history*f_dyn*x(N2+1:N2+1); ...
                   lam_rat*Phi(DWT_invert(x(1:N2)))];
            Ab = @(x) [lam_rat*DWT_apply(Phit(x((1+N2+1):end))) + gamma_hat*x((1+1):(1+N2)); ...
                   lambda_historyb*x(1:1) - lambda_history*(f_dyn')*x((1+1):(1+N2))];
            A = linop_handles([M+N2+1, N2+1], Af, Ab, opt_set);
            % Optimize the BPDN objective function with TFOCS
            res = solver_L1RLS( A, [zeros(N2,1); MEAS_SIG(:, :, kk)],  [lambda_b*ones(N2,1);0], zeros(N2+1, 1), opts );
        end
    end
    
    im_res  = DWT_invert(real(res(1:N2)));
    dyn_res = zeros(N_b,1);
    dyn_res(max_dyn) = res(N2+1);
    
    % Save reconstruction results
    if lambda_val <= lambda_b
        coef_dcs(:, :, kk)  = res(1:N2);
        bcoef_dcs(:, :, kk) = (lambda_val/lambda_b)*dyn_res;
    elseif lambda_val > lambda_b
        coef_dcs(:, :, kk)  = (lambda_b/lambda_val)*res(1:N2);
        bcoef_dcs(:, :, kk) = dyn_res;
    end
    recon_dcs(:, :, kk) = im_res;
    if rMSE_calc_opt == 1
