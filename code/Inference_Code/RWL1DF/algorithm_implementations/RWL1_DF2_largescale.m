function [varargout] = RWL1_DF2_largescale(varargin)
...
%     weights = 1;
    weights = (1 + (w1 - 1)*(abs(vid_coef_drw(:, :, 1))>0.01))*rwl1_mult./(rwl1_reg);
    for nn = 1:10
        % M-Step: Solve BPDN
        ...
        res = res./weights;
        % E-Step: Update weights
	supp_loc = abs(vid_coef_drw(:, :, kk-1))>0.01;
        fprintf('Percent coefficeints predicted active is %d\n', 100*sum(supp_loc)/numel(supp_loc) )
        weights = (1 + (w1 - 1)*supp_loc)*rwl1_mult./(abs(real(res)) + rwl1_reg);
