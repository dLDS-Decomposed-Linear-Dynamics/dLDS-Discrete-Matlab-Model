...
for kk = 1:num_frames
    ...
    for nn = 1:10
	    ...
	    if nn == 1
		    ...
		    resd = resd/sum(res2.^2);
	    end
            fprintf('Finished RW iteration %d. rMSE is %f, delta move is %f\n', nn, temp_rMSE, resd)
	    % fprintf('\t Stats: Mean = %f, Max = %f, min = %f\n', mean(abs(res)), max(abs(res)), min(abs(res)))
        else
    ...
    if rMSE_calc_opt == 1
        ...
        vid_PSNR_rwcs(kk) = psnr(real(vid_recon_rwcs(:, :, kk)), TRUE_VID(:, :, kk), 2);
        TIME_ITER = toc;
