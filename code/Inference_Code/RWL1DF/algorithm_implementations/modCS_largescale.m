function [varargout] = modCS_video(varargin)

% [vid_coef_dcs, vid_recon_dcs, vid_rMSE_dcs, vid_PSNR_dcs] = ...
%        mod_CS_video(MEAS_SIG, MEAS_SEL, DYN_FUN, DWTfunc, param_vals, ...
%        TRUE_VID)
%
%   The inputs are:
% 
...
%     x_pred  = DWT_apply(f_dyn(vid_recon_dcs(:, :, kk-1)));
    weights = vid_coef_dcs(:, :, kk-1)~=0;
    Wapply  = @(z) weights(:).*z(:);
    Wtrans  = @(z) weights(:).*z(:);
    W       = linop_handles([N2,N2], Wtrans, Wapply );
    
    mu = 0.1;
    x0 = [];
    z0 = [];
    opts.continuation = true;
    res = solver_sBPDN_W(A, W,...
      MEAS_SIG(:, :, kk), EPS, mu, x0, z0, opts );
