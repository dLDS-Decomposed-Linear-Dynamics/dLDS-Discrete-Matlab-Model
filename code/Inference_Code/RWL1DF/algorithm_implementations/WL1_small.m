function [x_est, lambda_est, time_tot] = WL1_small(varargin)

% [x_est, lambda_est, time_tot] = RWL1_DFmult(y_meas, G_n, F_n, ...
...
lambda_EM = lambda_pred;

[z_EM, ~, ~, ~] = BPDN_homotopy_function(G_n(:, :, 1)*diag(1./lambda_EM), ...
        y_meas(:, 1), tau, maxiter);
...
for index = 2:num_iter
    ...
    lambda_pred = abs(F_n(:, :, index)*x_est(:, index-1));
    supp_est = abs(lambda_pred) >= 0.001;
    fprintf('Support size = %f. Mean Abs = %f, mean vals = %f.\n', sum(supp_est)/numel(supp_est), mean(F_n(:, :, index)*x_est(:, index-1)), mean(x_est(:, index-1)))
    % Initalize current estimates of x and lambda
    lambda_EM = ((0.1 - 1)*supp_est + 1).*(1./(beta));
        
    % M-step: Solve weighted BPDN
    [z_EM, ~, ~, ~] = BPDN_homotopy_function(G_n(:, :, index)*diag(1./lambda_EM), ...
    y_meas(:, index), tau, maxiter);
    x_EM = z_EM./lambda_EM;
