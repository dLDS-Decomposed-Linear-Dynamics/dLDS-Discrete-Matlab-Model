...
for nn = 1:10
    tIter = tic;
    % M Step: Solve BPDN
    Af = @(x) Phi(DWT_invert((1./NORM_vals(:, 1)).*(x./weights)));
    Ab = @(x) (1./NORM_vals(:, 1)).*DWT_apply(Phit(x))./weights;
    A = linop_handles([M, N2], Af, Ab, opt_set);
    res = solver_L1RLS( A, reshape(MEAS_SIG(:, :, 1), [], 1), lambda_val, res2, opts );
    res = res./weights;
    % E-step: Update weights
    weights = 1./(abs(real(res)) + rwl1_reg);

    if rMSE_calc_opt == 1
	res_diff = sum((res2 - res).^2)/sum(res.^2);
	res2 = res;
        im_res = DWT_invert(real(res./NORM_vals(:, 1)));
        temp_rMSE = sum(sum((im_res - TRUE_VID(:, :, 1)).^2))/sum(sum(TRUE_VID(:, :, 1).^2));
        fprintf('Finished RW iteration %d, rMSE = %f, estimate diff = %f, time = %f.. \n', nn, temp_rMSE, res_diff, toc(tIter))
    else
...
if rMSE_calc_opt == 1
    ...
    vid_PSNR_drw(1) = psnr(vid_recon_drw(:, :, 1), TRUE_VID(:, :, 1), 2);
    TIME_ITER = toc;
...
for kk = 2:num_frames
    ...
    % Predict weights
    if df_only == 1
        x_pred = f_dyn(vid_coef_drw(:, :, kk-1));
    else
        x_pred = NORM_vals(:, kk).*DWT_apply(f_dyn(vid_recon_drw(:, :, kk-1)));
    end
    ...
    for nn = 1:10
        tIter = tic;
        % M-Step: Solve BPDN
        Af = @(x) Phi(DWT_invert((1./NORM_vals(:, kk)).*x./weights));
	    Ab = @(x) (1./NORM_vals(:, kk)).*DWT_apply(Phit(x))./weights;
        A = linop_handles([M, N2], Af, Ab, opt_set);
        res = solver_L1RLS(A, MEAS_SIG(:, :, kk), lambda_val, zeros(N2, 1), opts );
        res = res./weights;
        % E-Step: Update weights
	    w2      = 1*sum(res(:).^2)/sum(x_pred(:).^2);
        weights = rwl1_mult./(w1*abs(real(res)) + w2*abs(x_pred) + rwl1_reg);
        ...
        if rMSE_calc_opt == 1
            ...
            fprintf('Finished RW iteration %d, rMSE = %f, estimate diff = %f, time = %f, w2 = %f ... \n', nn, temp_rMSE, res_diff, toc(tIter), w2)
        else
            fprintf('Finished RW iteration %d.\n', nn)
        end
    end
    ...
    if rMSE_calc_opt == 1
        ...
        vid_PSNR_drw(kk) = psnr(real(vid_recon_drw(:, :, kk)), TRUE_VID(:, :, kk), 2);
        TIME_ITER = toc;
