function [x_est, lambda_est, time_tot] = RWL1_DFmult(varargin)
...
for index = 2:num_iter
    ...
    supp_est = abs(lambda_pred) ~= 0; %>= 0.001;
%     fprintf('Support size = %f. Mean Abs = %f, mean vals = %f.\n', sum(supp_est), mean(F_n(:, :, index)*x_est(:, index-1)), mean(x_est(:, index-1)))
    % Initalize current estimates of x and lambda
    lambda_EM = zeros(dim_x, 1);
    x_EM = zeros(dim_x, 1);
    for index_EM = 1:num_EM;
        % E-step: Update lambda
        lambda_EM = ((0.1 - 1)*supp_est + 1).*(2./(beta + abs(x_EM)));
