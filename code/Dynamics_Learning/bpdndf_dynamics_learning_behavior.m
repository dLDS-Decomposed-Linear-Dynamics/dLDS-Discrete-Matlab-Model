function [D, F, varargout] = bpdndf_dynamics_learning_behavior(data_obj, F_init, D_init, Psi_init, behavior, inf_opts)
...
inf_opts = checkSizes(inf_opts, D_init, F_init, Psi_init);                           % Check the sizes of all the initializations
...
%         D = cell(size(data_obj,1),1);
        Psi = cellfun(@(x) initialize_dictionary(inf_opts.nF, x),inf_opts.nBhv,'UniformOutput',false);   % If necessary, initialize the dictionary
    else
        Psi = initialize_dictionary(inf_opts.nF, size(behavior{1},1));   % If necessary, initialize the dictionary
    end
...
for n_iters = 1:inf_opts.max_iters
    ...
    if inf_opts.AcrossIndividuals % EY added 9/6/23
        ...
        bhv_ex = cellfun(@(x) sampleSomeDataSeqs(inf_opts, data_type, sig_opts, x, ...
                                                           D_true, F_true),behavior,'UniformOutput',false); % EY added 7/9/24
    else
    ...
    if (inf_opts.Psi_update == true) % (~strcmp(inf_opts.special, 'noobs'))||   % EY added 7/8/24
        if inf_opts.AcrossIndividuals 
            for ii = 1:size(data_obj,1)                
                Psi{ii} = dictionary_update(cell2mat(bhv_ex{ii}), Psi{ii}, cell2mat(B_cell{ii}), ...
                                                         inf_opts.step_psi); % Update static dictionary
            end
        else
            Psi = dictionary_update(cell2mat(bhv_ex), Psi, cell2mat(B_cell), ...
                                                         inf_opts.step_psi); % Update static dictionary
        end
    end  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function inf_opts = checkSizes(inf_opts, D_init, F_init, Psi_init)
...
if (~isfield(inf_opts,'nBhv'))||isempty(inf_opts.nBhv)
    ...
    elseif isfield(sig_opts,'nBhv')
        inf_opts.nBhv = sig_opts.nBhv;
    else
        inf_opts.nBhv = 1;
    end
end
