...
elseif iscell(data_obj)&&(numel(size(data_obj{1}))==2)
    ...
    height_im = size(data_obj{1}, 2) - Ts + 1;                             % Get heights and widths for allowable starting indices for subimages
    start_ind = ceil(bsxfun(@times,rand(opts.N_ex,2),[height_im, num_im]));  % Pick random starting points
    for kk = 1:opts.N_ex
        x_im{kk} = data_obj{start_ind(kk,2)}(:,...
                            start_ind(kk,1):start_ind(kk,1)+Ts-1); % Pick out subimage and reshape it to a vector
        if (opts.ssim_flag)&&(sqrt(sum(x_im{kk}(:).^2))>0)            % If desired, normalize the sequence
                x_im{kk} = x_im{kk}./sqrt(sum(x_im{kk}(:).^2));            % Normalize the chosen sample, as long as the norm is not zero
        end
    end
