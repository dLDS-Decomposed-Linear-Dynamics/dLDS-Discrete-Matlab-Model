function [A_cell,B_cell] = parallel_bilinear_dynamic_inference(X, D, F, infer_hand, inf_opts, varargin)
...
parfor kk = 1:numel(X)
% for kk = 1:numel(X)
    [A_cell{kk},B_cell{kk}] = feval(infer_hand, X{kk}, D, F, inf_opts);    % Perform inference over each sequence independently
end
