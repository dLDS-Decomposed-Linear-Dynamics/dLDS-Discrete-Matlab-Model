...
elseif strcmp(opts.data_type, 'square')
    ...
    while sum(ix)~=0
        ...
        for index_in = 1:sum(ix)
            x_im = cat(2, x_im, reshape(data_obj(start_ind(1):start_ind(1)+opts.bl_size(1)-1,...
                start_ind(2):start_ind(2)+opts.bl_size(2)-1, start_ind(3)), [], 1)); % Pick out subimage and reshape it to a vector
        end
        if opts.ssim_flag == 1
