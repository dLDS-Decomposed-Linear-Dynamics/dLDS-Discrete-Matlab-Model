...
opts.save_every = 0;              % How many iterations to save after
...
% Dictionary options
opts.n_elem = 400;                   % Number of dictionary elements
opts.bl_size = 10;                   % Block Size (nxn)
opts.dep_size = 1;                  % Depth size for 'cube' data

% Iteration numbers
opts.iters = 100;                  % Number of learning iterations
opts.in_iter = 10000;                 % Number of internal iterations
...
learn_times = zeros(1, numel(n_meas));
learn_times_spmd = zeros(1, numel(n_meas));

for ii = 1:numel(n_meas)
    
    matlabpool('open', n_meas(ii))
