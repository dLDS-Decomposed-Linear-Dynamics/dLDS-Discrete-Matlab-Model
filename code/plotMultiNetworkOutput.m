function plotMultiNetworkOutput(tSel, dFF, A_cell, B_cell, groundTruthStates, F)



% tSel = 5; % selected trial
figure(1), cla; 
subplot(5,4,[1,4]), plot(dFF{tSel}.')       % ASC added 7/25
box off; xlabel('Time (frames)'); ylabel('Signal (GT)')
set(gca,'XLim',[1,size(dFF{tSel},2)]);
subplot(5,4,[5,8]), plot(A_cell{tSel}.')    % ASC added 7/25
set(gca,'XLim',[1,size(dFF{tSel},2)]);
box off; xlabel('Time (frames)'); ylabel('Reconstruction')

subplot(5,4,[9,12]), imagesc(abs(B_cell{tSel}))   % ASC added 7/25
set(gca,'XLim',[1,size(dFF{tSel},2)]);
...
% legend('C1','C2','C3','C4')
subplot(5,4,[13,16]), imagesc(groundTruthStates{tSel})   % ASC added 7/25
...
subplot(5,4,[17,20]), imagesc(allFs); 
